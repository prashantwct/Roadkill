import os
from app import app, db, Carcass

# ---------------------------------------------------------
# 1. DIAGNOSE: CHECK DATABASE DATA
# ---------------------------------------------------------
print("--- DIAGNOSTICS ---")
with app.app_context():
    total = Carcass.query.count()
    mapped = Carcass.query.filter(Carcass.latitude.isnot(None), Carcass.longitude.isnot(None)).count()
    
    print(f"Total Carcasses: {total}")
    print(f"Mappable (have GPS): {mapped}")
    
    if mapped == 0 and total > 0:
        print("⚠️ WARNING: You have records, but NONE have GPS coordinates.")
        print("   The map will be empty. Edit a carcass and add Latitude (e.g. 23.25) and Longitude (e.g. 77.41).")
    elif total == 0:
        print("⚠️ WARNING: No carcass records found at all.")

# ---------------------------------------------------------
# 2. FIX MAP TEMPLATE (Robust Data Injection)
# ---------------------------------------------------------
...
document.addEventListener("DOMContentLoaded", function() {
  ...
  data.forEach(d => {
      ...
      var marker = L.marker([d.lat, d.lng]).bindPopup(popupHtml);
...
print("✅ Updated templates/map.html with safe data loading.")
